...
use snafu::location;
use sqlparser::ast::{
    Distinct as SQLDistinct, Expr, GroupByExpr, LimitClause, ObjectName, OrderBy, OrderByKind,
    Query, SelectItem, SetExpr, TableFactor, TableWithJoins,
};
...
pub fn query_to_plan(query: Query, tables: &HashMap<String, ListingTable>) -> Result<LogicalPlan> {
    ...
    let plan = plan_from_limit(
        plan,
        query.limit_clause.and_then(|lm| match lm {
...
fn plan_from_order_by(plan: LogicalPlan, order_by: Option<OrderBy>) -> Result<LogicalPlan> {
    match order_by {
        ...
        Some(OrderBy {
            ...
            // if let OrderByKind::Expressions(exprs) = kind {}
            ...
            for expr in &exprs {
                ...
                if let Expression::Column(_) = sort_col {
                    ...
                    expression.push(sort(sort_col, true));
                }
            }
    ...
    match relation {
        TableFactor::Table { name, .. } => {
            ...
            let table_name = match idents.len() {
                1 => &idents[0].as_ident().expect("expected ident value").value,
                _ => {
                    return Err(Error::InvalidOperation {
